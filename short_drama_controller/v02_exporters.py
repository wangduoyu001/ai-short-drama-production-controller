...
def write_shot_csv(project: Project, path: Path) -> None:
    fields = [
        ...
        "beat_id 节拍编号", "shot_purpose 镜头目的", "scene_id 场景编号",
...
def write_sound_csv(project: Project, path: Path) -> None:
    fields = [
        "shot_id 镜头编号", "clip_id 单段编号", "clip_type 片段类型", "clip_duration_seconds 片段时长秒数", "beat_id 节拍编号",
        "speaker_mode 发声模式", "mouth_state 嘴型状态",
...
def write_producer_csv(project: Project, path: Path) -> None:
    fields = [
        "shot_id 镜头编号", "clip_id 单段编号", "clip_type 片段类型", "clip_duration_seconds 片段时长秒数", "beat_id 节拍编号",
        "this_clip_only 本段只拍", "reserved_for_later 后续保留",
